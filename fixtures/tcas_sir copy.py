def alt_sep_test(
    ...
    Alt_Layer_Value: int,           # int Alt_Layer_Value;   (global, 0-3)
    ...
    Inlining map:
      ALIM()                 -> (400 if Alt_Layer_Value==0 else 500 if ==1 else 640 if ==2 else 740)
      Inhibit_Biased_Climb() -> (Up_Separation + 100 if Climb_Inhibit != 0 else Up_Separation)
    ...
    if (High_Confidence                                          # enabled: High_Confidence
        ...
        if (Up_Separation + 100 if Climb_Inhibit != 0 else Up_Separation) > Down_Separation:  # upward_preferred
            ...
            if (
                    ((not (Own_Tracked_Alt < Other_Tracked_Alt)                             # NCBC_up: !(Own_Below_Threat())
                      or ((Own_Tracked_Alt < Other_Tracked_Alt)                             # NCBC_up: Own_Below_Threat()
                          and not (Down_Separation >= (400 if Alt_Layer_Value == 0 else     # NCBC_up: !(Down_Separation >= ALIM())
                                                       500 if Alt_Layer_Value == 1 else
                                                       640 if Alt_Layer_Value == 2 else
                                                       740))))
                    ...
                    ((Own_Tracked_Alt < Other_Tracked_Alt)                                  # NCBD_up: Own_Below_Threat()
                     ...
                     and (Down_Separation >= (400 if Alt_Layer_Value == 0 else              # NCBD_up: Down_Separation >= ALIM()
                                              500 if Alt_Layer_Value == 1 else
                                              640 if Alt_Layer_Value == 2 else
                                              740))
            ...
            if (
                    ((Other_Tracked_Alt < Own_Tracked_Alt)                                  # NCBC_down: Own_Above_Threat()
                     ...
                     and (Up_Separation >= (400 if Alt_Layer_Value == 0 else                # NCBC_down: Up_Separation >= ALIM()
                                            500 if Alt_Layer_Value == 1 else
                                            640 if Alt_Layer_Value == 2 else
                                            740))
                     and (Own_Tracked_Alt < Other_Tracked_Alt))                             # need_upward_RA: && Own_Below_Threat()
                    ...
                    ((not (Other_Tracked_Alt < Own_Tracked_Alt)                             # NCBD_down: !(Own_Above_Threat())
                      or ((Other_Tracked_Alt < Own_Tracked_Alt)                             # NCBD_down: Own_Above_Threat()
                          and (Up_Separation >= (400 if Alt_Layer_Value == 0 else           # NCBD_down: Up_Separation >= ALIM()
                                                 500 if Alt_Layer_Value == 1 else
                                                 640 if Alt_Layer_Value == 2 else
                                                 740))))
                     and (Other_Tracked_Alt < Own_Tracked_Alt))                             # need_downward_RA: && Own_Above_Threat()
